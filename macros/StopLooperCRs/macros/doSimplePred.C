...
void doSimplePred() {
 ...
 //float in_ftopW_aunc[NSR] = { 0.591,  1.537,  4.942,  10.};
  ...
  for (int isr=0; isr<NSR; ++isr) {
    ...
    for (int il = 0; il<NLEP; il++) {
      h_ttdl_wtrk[il] = (TH1F*)mc_sl[TTDL]->Get(Form("%s%s%s%s",histoname.c_str(),wtrk_tag.c_str(),
						     selection[isr].c_str(),
						     leptag[il].c_str()));
      h_ttdl_wtrk[il]->SetName(Form("h_mc_ttdl%s%s", wtrk_tag.c_str(),leptag[il].c_str()));
      h_ttdl_wtau[il] = (TH1F*)mc_sl[TTDL]->Get(Form("%s%s%s%s",histoname.c_str(),wtau_tag.c_str(),
						     selection[isr].c_str(),
						     leptag[il].c_str()));
      h_ttdl_wtau[il]->SetName(Form("h_mc_ttdl%s%s", wtau_tag.c_str(),leptag[il].c_str()));
    }
      
    float frac_dl_lepveto = ( h_ttdl_wtrk[MUO]->GetBinContent(TAIL)
			      +h_ttdl_wtrk[ELE]->GetBinContent(TAIL) )/(mc_dl[MUO]+mc_dl[ELE]);
    float e_frac_dl_lepveto = sqrt(frac_dl_lepveto*(1-frac_dl_lepveto)/(mc_dl[MUO]+mc_dl[ELE]));
    float frac_dl_tauveto = ( h_ttdl_wtau[MUO]->GetBinContent(TAIL)
			      +h_ttdl_wtau[ELE]->GetBinContent(TAIL) )/(mc_dl[MUO]+mc_dl[ELE]);
    float e_frac_dl_tauveto = sqrt(frac_dl_tauveto*(1-frac_dl_tauveto)/(mc_dl[MUO]+mc_dl[ELE]));
	...
	sqrt( pv_data[il] + pow(e_pv_rare[il],2) +
      ...
      if (doverbose) cout<<"PREVETO "<<pv_sf[il][isr]<<" pm "<< e_pv_sf[il][isr]<<endl;
    }
	...
	: 0.4;
      ...
      e_pred_ttdl[il][isr] = pred_ttdl[il][isr] * sqrt( pow(e_kfact,2) +
							pow((e_2ndlepveto*frac_dl_lepveto),2) +
							pow((e_2ndtauveto*frac_dl_tauveto),2) +
							pow(e_dl_vars[isr],2) +
    ...
    e_pred_ttdl_comb[isr] = pred_ttdl_comb[isr] * 
      sqrt( pow(e_kfact,2) +
	    ...
	    pow( ((e_mc_dl[MUO]+e_mc_dl[ELE])/(mc_dl[MUO]+mc_dl[ELE])),2) );
    //Missing the Wjets systematic and a piece of the rare MC
    float pred_total_comb = pred_top1l_comb[isr]+pred_wjets_comb[isr]+pred_rare_comb[isr]+pred_ttdl_comb[isr];
    float e_pred_total_comb = sqrt( pow( (e_pred_top1l_comb[isr]+e_pred_wjets_comb[isr]),2)+
    ...
    //rare down
    //total without the rare component
    float pred_total_raredw_norare_comb = pred_total_raredw_norare[MUO] + pred_total_raredw_norare[ELE];

    //rare up
    //total without the rare component
    float pred_total_rareup_norare_comb = pred_total_rareup_norare[MUO] + pred_total_rareup_norare[ELE];
    ...
    e_pred_total_final_comb[isr] = sqrt( pow((e_pred_top1l_comb[isr]+e_pred_wjets_comb[isr]),2)+
