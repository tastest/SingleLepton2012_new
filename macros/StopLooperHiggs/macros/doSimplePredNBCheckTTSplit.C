...
//settings go here
const int NSAMPLE = 3;
char * selection[NSAMPLE] = {"_mtpeak_1l2b_g4j", "_mtpeak_1l3b_g5j", "_mtpeak_1l4b_g4j"};    
...
float ttslpred[NSAMPLE]; 
float ettslpred[NSAMPLE];
...
void doSimplePredNBCheckTTSplit(bool doaltttbar = false) {
...
void runSimplePred(char* ttbar_tag, bool issyst){
  ...
  float rmbb[NSAMPLE];
  float ermbb[NSAMPLE];
  cout<<"${\\rm R}_{{\\rm M}_{{\\rm b}\\bar{{\\rm b}}}}^{\\rm MC}$  ";
  for (int isr=0; isr<NSAMPLE; ++isr) {
  ...
  // float sf_rmbb = rmbb_dt[0]/rmbb[0];
  // float esf_rmbb = addSqr( (ermbb_dt[0]/rmbb_dt[0]), (ermbb[0]/rmbb[0]) )*sf_rmbb; 
  ...
  for (int isr=0; isr<NSAMPLE; ++isr) {
    if (isr==0) {
      ttslpred[isr]  = h_dt_cor[isr]->GetBinContent(i_ctr)*rmbb[isr];
      ettslpred[isr] = sqrt( pow( (h_dt_cor[isr]->GetBinError(i_ctr)/h_dt_cor[isr]->GetBinContent(i_ctr)), 2) + 
			   pow( (ermbb[isr]/rmbb[isr]), 2) ) * ttslpred[isr];
    } else {
      ttslpred[isr]  = h_dt_cor[isr]->GetBinContent(i_ctr)*rmbb[isr]*sf_rmbb[0];
      ettslpred[isr] = sqrt( pow( (h_dt_cor[isr]->GetBinError(i_ctr)/h_dt_cor[isr]->GetBinContent(i_ctr)), 2) + 
			   pow( (ermbb[isr]/rmbb[isr]), 2) + pow( (esf_rmbb[0]/sf_rmbb[0]),2) ) * ttslpred[isr];
    }
    printf("& $%.2f \\pm %.2f$ ",ttslpred[isr], ettslpred[isr]);    
  }
  printf(" \\\\\n");
  ...
  for (int isr=0; isr<NSAMPLE; ++isr) {
    pred[isr] = ttslpred[isr] + ttdlpred[isr] + rarepred[isr];
    epred[isr] = addSqr( ettslpred[isr], ettdlpred[isr]);
  ...
  for (int isr=0; isr<NSAMPLE; ++isr) {
    ...
    eclosure[isr] = h_dt[isr]->GetBinContent(i_sig)>0. ? 
      addSqr(h_dt[isr]->GetBinError(i_sig)/h_dt[isr]->GetBinContent(i_sig), epred[isr]/pred[isr]) * closure[isr] : 0.;
  ...
  // for (int i=0; i<NSAMPLE; ++i) 
  //   err[i]=0.2;
  float err[NSAMPLE] = {0., 0.1, 0.3};

  TGraphErrors* gclose = new TGraphErrors(NSAMPLE,x,closure,xerr,eclosure);
  ...
  TH2F* hdummy = new TH2F("hdummy","",NSAMPLE,0,NSAMPLE,100,0.2,1.8);
  for (int i=0; i<NSAMPLE; ++i) 
  ...
  hdummy->GetYaxis()->SetTitle("data / MC (M_{bb} in)");
  ...
  c1->Print("plots/nbcheck.pdf");
  c1->Print("plots/nbcheck.png");  
}
