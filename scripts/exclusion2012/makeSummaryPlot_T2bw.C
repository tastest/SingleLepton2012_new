...
void makeSummaryPlot_T2bw( bool doBDT = true , bool print = false){
  ...
  leg->AddEntry(c_T2bw       ,"#tilde{t}#rightarrowb #tilde{#chi}^{+}_{1} (observed)"   ,"l");
  leg->AddEntry(c_T2bw_exp   ,"#tilde{t}#rightarrowb #tilde{#chi}^{+}_{1} (expected)"   ,"l");
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  leg->Draw();
  ...
  t->SetTextAngle(0);
  t->SetTextSize(0.05);
  t->DrawLatex(0.15,0.05,"m_{#chi_{1}^{#pm}} = 0.5 m_{ #tilde{t}} + 0.5 m_{#chi_{1}^{0}}");
