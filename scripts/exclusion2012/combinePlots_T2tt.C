...
void combinePlots_T2tt(char* sample = "T2tt" , int x = 1, bool doBDT = true, char* pol = "" , bool print = false){

  int colors[]={1,1,1,1,2,3,4,kOrange,6};
  ...
  char* outfilename = Form("rootfiles/%s%s_combinePlots_T2tt%s%s%s%s.root",sample,xchar,suffix,pol,BDTchar,nminchar);
  ...
  // exclusion contours for different branching fractions
  TH2F* hR_BF[9];
  TH2F* hR_BF_smallDM[9];

  for( int i = 0 ; i < 9 ; i++ ){
    hR_BF[i]          = exclusionContour(hxsec_best_largeDM       ,sample,doBDT,nsmooth,"nominal" , pow((i+1)*0.1,2) );
    hR_BF_smallDM[i]  = exclusionContour(hxsec_best_smallDM       ,sample,doBDT,nsmooth,"nominal" , pow((i+1)*0.1,2) );
    hR_BF[i]->SetLineColor(colors[i]);
    hR_BF_smallDM[i]->SetLineColor(colors[i]);
  }
  ...
  TCanvas *can1 = new TCanvas("can1","",650,600);
  can1->cd();
  ...
  for( int i = 0 ; i < 9 ; i++ ){
    if( i > 3 ) hR_BF[i]->Draw("CONT3SAMEC");
    if( i > 4 ) hR_BF_smallDM[i]->Draw("CONT3SAMEC");
  }
  ...
  t->SetTextSize(0.04);
  t->DrawLatex(0.65,0.85,"Observed limits");
  t->SetTextSize(0.037);

  // TLegend *leg = new TLegend(0.75,0.5,0.94,0.83);
  // leg->SetTextSize(0.035);
  // leg->AddEntry(hR       ,"BF = 1.0"   ,"l");
  // for( int i = 8 ; i > 3 ; i-- ){
  //   leg->AddEntry(hR_BF[i]  , Form("BF = %.1f",(i+1)*0.1)   ,"l");
  // }

  TLegend *leg = new TLegend(0.65,0.58,0.94,0.83);
  leg->SetTextSize(0.025);
  leg->AddEntry(hR       ,"BF(#tilde{t} #rightarrow t#tilde{#chi}_{1}^{0}) = 1.0"   ,"l");
  for( int i = 8 ; i > 3 ; i-- ){
    leg->AddEntry(hR_BF[i]  , Form("BF(#tilde{t} #rightarrow t#tilde{#chi}_{1}^{0}) = %.1f",(i+1)*0.1)   ,"l");
  }
  ...
  */
  ...
  if( TString(sample).Contains("T2tt") ){
    ...
    t->SetTextAngle(55);
    t->SetTextSize(0.04);
    t->DrawLatex(0.36,0.51,"m_{#tilde{t}} - m_{#tilde{#chi}^{0}_{1}} = m_{W}");
    t->DrawLatex(0.46,0.51,"m_{#tilde{t}} - m_{#tilde{#chi}^{0}_{1}} = m_{t}");
  ...
  if( print ){
    can1->Print(Form("plots/combinePlots_T2tt_%s%s%s%s%s%s.pdf"               ,sample,xchar,suffix,pol,BDTchar,nminchar));
    can2->Print(Form("plots/combinePlots_T2tt_%s%s%s%s%s%s_bestRegion.pdf"    ,sample,xchar,suffix,pol,BDTchar,nminchar));
    if( plotExcludedPoints ) can3->Print(Form("plots/combinePlots_T2tt_%s%s%s%s%s%s_excludedPoints.pdf",sample,xchar,suffix,pol,BDTchar,nminchar));
  }
...
void doAll(){
  ...
  combinePlots_T2tt("T2tt", 1,false,""     ,true);
  // combinePlots_T2tt("T2tt", 1,false,"left" ,true);
  // combinePlots_T2tt("T2tt", 1,false,"right",true);

  combinePlots_T2tt("T2tt", 1,true,""     ,true);
  // combinePlots_T2tt("T2tt", 1,true,"left" ,true);
  // combinePlots_T2tt("T2tt", 1,true,"right",true);

  //--------------------------
  // T2bw madgraph
  //--------------------------

  combinePlots_T2tt("T2bw_MG",25,false,"T2BW_SS",true);
  combinePlots_T2tt("T2bw_MG",50,false,"T2BW_SS",true);
  combinePlots_T2tt("T2bw_MG",75,false,"T2BW_SS",true);

  combinePlots_T2tt("T2bw_MG",25,true,"T2BW_SS",true);
  combinePlots_T2tt("T2bw_MG",50,true,"T2BW_SS",true);
  combinePlots_T2tt("T2bw_MG",75,true,"T2BW_SS",true);

  //--------------------------
  // T2bw pythia
  //--------------------------

  // combinePlots_T2tt("T2bw",25,false,"",true);
  // combinePlots_T2tt("T2bw",50,false,"",true);
  // combinePlots_T2tt("T2bw",75,false,"",true);

  // combinePlots_T2tt("T2bw",25,true,"",true);
  // combinePlots_T2tt("T2bw",50,true,"",true);
  // combinePlots_T2tt("T2bw",75,true,"",true);
  ...
  for( int i = 0 ; i < 9 ; i++ ){
    combinePlots_T2tt("T2bw_MG",25,true,weights[i],true);
    combinePlots_T2tt("T2bw_MG",50,true,weights[i],true);
    combinePlots_T2tt("T2bw_MG",75,true,weights[i],true);

    combinePlots_T2tt("T2bw_MG",25,false,weights[i],true);
    combinePlots_T2tt("T2bw_MG",50,false,weights[i],true);
    combinePlots_T2tt("T2bw_MG",75,false,weights[i],true);
  }
