...
void makeSummaryPlot_T2tt( bool doBDT = true , bool print = false){
  ...
  TLegend *leg = new TLegend(0.6,0.75,0.90,0.88);
  ...
  leg->AddEntry(c_T2tt       ,"#tilde{t}#rightarrowt #tilde{#chi}^{0}_{1} (observed)"   ,"l");
  leg->AddEntry(c_T2tt_exp   ,"#tilde{t}#rightarrowt #tilde{#chi}^{0}_{1} (expected)"   ,"l");
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  leg->Draw();
  ...
  t->DrawLatex(0.46,0.55,"m_{#tilde{t}} - m_{#tilde{#chi}_{1}^{0}} = m_{t}");
  t->DrawLatex(0.35,0.55,"m_{#tilde{t}} - m_{#tilde{#chi}_{1}^{0}} = m_{W}");

  if( print ){
    can1->Print(Form("plots/makeSummaryPlot%s_T2tt.pdf",BDTchar));
    can1->Print(Form("plots/makeSummaryPlot%s_T2tt.png",BDTchar));
  }
